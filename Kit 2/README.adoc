...
For the A and B registers, we will use red LEDs and for the instruction register we will use blue and yellow LEDs. The A and B registers will be built on the left side of the breadboard and the instruction register will be built on the right side of the breadboard. This is so that things line up better for the full computer.
...
The eighth LED needs to go from *ground `-`* to *pin 6 (Q~4~)* of the second *74LS173 4-bit D register*
